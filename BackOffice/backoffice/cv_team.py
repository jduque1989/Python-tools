...
def login(driver, url, username, password):
    ...
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, "myusername"))
    ).send_keys(username)
...
def fetch_and_calculate_total_sum(driver):
    ...
    # Print header
    print(
        f"{'Left Branch Country':<20}{'Value':>10} | {'Right Branch Country':<20}{'Value':>10}"
    )
    print("-" * 70)
    # Print countries and values side by side
    for i in range(max_length):
        left_country = (
            country_data["left"][i] if i < len(country_data["left"]) else ("", "")
        )
        right_country = (
            country_data["right"][i] if i < len(country_data["right"]) else ("", "")
        )
        print(
            f"{left_country[0]:<20}{left_country[1]:>10} | {right_country[0]:<20}{right_country[1]:>10}"
        )
    ...
    print(
        f"{'Left Branch Sum:':<20}{left_sum:>10} | {'Right Branch Sum:':<20}{right_sum:>10}"
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "csv_file",
        nargs="?",
        default="./teamcv/team_code.csv",
        help="The name of the CSV file to process (default: ./teamcv/team_code.csv)",
    )
...
def loop_cv(driver, filename, path_to_create):
    ...
    for i in range(0, len(IDs)):
        ...
        click_element(
            driver,
            By.XPATH,
            f"//div[contains(@class, 'ng-binding') and contains(text(), '{ids1}')]",
        )
        click_element(
            driver,
            By.XPATH,
            "//button[contains(@class, 'GE_ItemBtn_FullWidth') and contains(text(), 'Resumen del Ciclo Actual')]",
        )
...
def main():
    try:
        with initialize_driver() as driver:
            ...
            login(
                driver,
                "https://colombia.ganoexcel.com/Downline.aspx",
                username,
                password,
            )
        ...
        print(
            "We've found a connection problem, please try again. The operation timed out."
        )
    except WebDriverException as e:
        if "ERR_INTERNET_DISCONNECTED" in str(e) or "ERR_CONNECTION_TIMED_OUT" in str(
            e
        ):
